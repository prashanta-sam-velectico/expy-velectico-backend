...
$("#submit").on('click',function(){
  ...
  JsonPostData("admin/auth",flagData);
});
...
if(url.endsWith('/key-data'))
{
  $('#key-data').addClass('active-menu')
}


if(url.endsWith('/celebrity'))
{
  $('#celebrity').addClass('active-menu')
...
{
  $('#purchaseMade').addClass('active-menu')
...
{
  $('#discarded-premium-features').addClass('active-menu')
...
$(document).on('click', ".changeUrl", function() {
  ...
  {
    $('#create-discount').addClass('active-menu')
...
if(url.endsWith('/referral-ranking') || url.endsWith('/referral-ranking/'))
{
  ...
  $('#referral-ranking').addClass('active-menu')
...
//MS-3  B-1
function openAlertModal(event,id,type,name,landingImg='',isForLandingPage=0)
{
...
var checked=event.checked;
  ...
  var lnd_file =  document.getElementById('lnd_file');
  if (typeof(lnd_file) != 'undefined' && lnd_file != null)
  {    
    document.getElementById("lnd_file").remove();
    document.getElementById("title_lnd_file").remove();      
  }
  var view_file =  document.getElementById('view_file');
  if (isExistElement_JS(view_file)==true)
  {                   
    document.getElementById("view_file").remove();                                 
  }

  var lbl_bio =  document.getElementById('lbl_bio');
  if (isExistElement_JS(lbl_bio)==true)
  {                   
    document.getElementById("lbl_bio").remove();                                 
  }
  var txt_bio =  document.getElementById('txt_bio');
  if (isExistElement_JS(txt_bio)==true)
  {                   
    document.getElementById("txt_bio").remove();                                 
  }
  ...
  if(type=='B') 
  {
      ...
      $('#alert_title').html(title +" "+ name +"'s Profile ?");  
  } 
  else if(type=='V')
  {
      if(checked==true)
  ...
  else if(type=='L')
  {
      ...
      fileUpload.id = "lnd_file";    
      fileUpload.setAttribute("style", "margin-bottom: 5px;");
      fileUpload.setAttribute('onchange',  'uploadFile(this);');
      var p = document.createElement("p");
      p.setAttribute("style", "font-size: 15px; font-weight: bold;");
      p.innerHTML='Upload image for landing page (Dimensions : 150 x 215 px) :';
      p.id='title_lnd_file';
      document.getElementById('modelBody').appendChild(p);
      document.getElementById('modelBody').appendChild(fileUpload);

      if(isForLandingPage==0) 
      {
        var lbl_bio = document.createElement("span");
        lbl_bio.setAttribute("style", "margin-top: 10px;");
        lbl_bio.innerHTML='Bio for landing page :';
        lbl_bio.id='lbl_bio';
        var txt_bio = document.createElement("input");
        txt_bio.id="txt_bio"
        txt_bio.name="txt_bio"
        txt_bio.class="txt_bio"
        txt_bio.setAttribute('placeholder',"Enter bio for landing page");
        document.getElementById('modelBody').appendChild(lbl_bio);
        document.getElementById('modelBody').appendChild(txt_bio);

      }
      

        if(checked==true)
          $('#alert_title').html(" set up "+ name +"'s Profile in landing page?");  
        else
          $('#alert_title').html(" remove  "+ name +"'s Profile from landing page?");  
       if(landingImg.length > 0 && isForLandingPage==1)
       {
          var a = document.createElement('a');
          a.id="view_file";
          a.href = base_url+'adm/uploads/'+landingImg;
          a.setAttribute('target', '_blank');  
          a.innerText='View Image';                
          document.getElementById('modelBody').appendChild(a);
       }
 
       $('#btn_ok_alert').attr('data-landing',isForLandingPage);
  }
...
{
  const fileList = e.files; /* now you can work with the file list */
  ...
  var filePath = e.value;
    ...
    var allowedExtensions = /(\.jpg|\.jpeg|\.png)$/i;
      ...
      let file_size = e.files[0].size/1024;

      console.log(file_size);

      let file_name = e.files[0].name;
      let file_type = e.files[0].type;
      ...
      if(file_size > maxFileSize)
      {
        ...
        var JM_ID = document.getElementsByClassName("JM_ID")[0].value;
        let formData = new FormData()
        formData.append("sampleFile", file)
        formData.append("JM_ID", JM_ID)
        let API_url=base_url+"admin/landingImageFromAdmin";

      $('#upload_msg').show();
          $('#upload_msg').text('Uploading...');

       $('#btn_ok_alert').attr('disabled',true);
        ...
        fetch(API_url, {
          ...
          return response.json();
        }).then(data => 
            {
              if(data.status===1)          
              { 
               // alert("Landing page image uploaded successfully.");

                $('#upload_msg').text('Image is uploaded, press okay to save');
                $('#btn_ok_alert').attr('disabled',false);
               ...
               $('#upload_msg').hide();
                var a = document.createElement('a');
                a.id="view_file";
                a.href = base_url+'adm/uploads/'+data.url;
                a.setAttribute('target', '_blank');  
                a.innerText='View Image';                
                document.getElementById('modelBody').appendChild(a);
            }
            else
                  $('#upload_msg').text('Failed to upload image');
       });
  ...
  var id=$('.JM_ID').val(); 
   ...
   hideModal("alertModal");
 ...
 else if(type=='L')
    {
       ...
       //  displayLanding=parseInt($('#btn_ok_alert').attr('data-landing'));

          var chk= $("#priority_"+id).prop("checked");  
          let txt_bio = $("#txt_bio").val();   
          $('#upload_msg').hide();      
          $('#upload_msg').text('');
          if(chk == true)
          {
            if(txt_bio.length ==0 || txt_bio.length > 100)
            {
              $('#upload_msg').show();
              $('#upload_msg').css('color','red')
              $('#upload_msg').text('Enter bio for landing page within 100 character');
              return false;
            }
          }
                      ...
                      return response.json();
                    }).then(data => 
                        {
                          ...
                          { 
                            ...
                            msg="<strong> Success! </strong> Payout process is successfully completed";
                            $("#alertcustom").css('display','block');
                            $("#alertcustom").html(msg);
                            hideAlert("alertcustom");
...
//========================== MS3 B3
if(url.endsWith('/approve-decline-request') || url.endsWith('/approve-decline-request/'))
{

  //$('#referral-ranking').addClass('active-menu')
  $('#tbl_dec_request').dataTable();
  CreateTable_OrderBy('tbl_dec_request',0);
}

$(document).on('click', ".approve", async function(e) 
{
  event.preventDefault();
  let BM_ID=$(this).attr('data-bm');
  let JM_ID=$(this).attr('data-jm');
  let isApprove=parseInt($(this).attr('data-btn'));
  let Tax=parseFloat($(this).attr('data-tax'));
	let Fee=parseFloat($(this).attr('data-fee'));
	let Actual_Price=parseFloat($(this).attr('data-actual'));
	let BM_Purchase_Amt=parseFloat($(this).attr('data-amt'));
  let isFree=parseInt($(this).attr('data-isfree'));

  if(typeof isApprove=='undefined' || isNaN(isApprove))
  {
    ...
    let jsonData = {
      ...
      BM_Purchase_Amt:BM_Purchase_Amt,
      isFree:isFree
  };
 
  let flag =encryptJson(jsonData);   
  console.log(flag)
  const JSONdata = {
   flag:flag
};
...
const data=await JsonAPI('approve-decline-request',JSONdata);
